...
/// Prime Agent CLI status detection via tmux pane parsing.
/// Prime Agent is built on pi and shares the same TUI patterns:
/// braille spinner glyphs for running, "esc to interrupt" for active work,
/// and a `>` prompt when waiting for input.
pub fn detect_prime_agent_status(raw_content: &str) -> Status {
    let clean = strip_ansi(raw_content);
    let non_empty_lines: Vec<&str> = clean.lines().filter(|l| !l.trim().is_empty()).collect();
...
",
    ...
    // A parked input prompt outranks activity words: a `>` or `prime-agent>`
    // prompt at rest must not flip back to Running from lingering prose.
    if matches_input_prompt(&non_empty_lines, 5, &["prime-agent>"]) {
        return Status::Waiting;
    }
...
↑45k ↓11k $0.009 9.6%/500k (auto)                    gpt-5.5 • medium\n";
    ...
    #[test]
    fn test_detect_prime_agent_status_running() {
        assert_eq!(detect_prime_agent_status("generating ⠋"), Status::Running);
        assert_eq!(detect_prime_agent_status("loading ⠹"), Status::Running);
        assert_eq!(
...
esc to interrupt"
        ...
        assert_eq!(
            detect_prime_agent_status("thinking about code"),
            Status::Running
        );
        assert_eq!(
            detect_prime_agent_status("reading file.ts"),
            Status::Running
        );
    }

    #[test]
    fn test_detect_prime_agent_status_waiting() {
        assert_eq!(
            detect_prime_agent_status(
                "done
>"
...
>"
    ...
    #[test]
    fn test_detect_prime_agent_status_idle() {
    ...
    #[test]
    fn test_detect_prime_agent_status_running_spinner_footer() {
        assert_eq!(detect_prime_agent_status(PI_RUNNING_PANE), Status::Running);
    }
